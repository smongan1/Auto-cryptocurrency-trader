...
tot_fin=[];
plotters=[];
start_money=25; current_money=start_money; cnt=0; fake_prices=[]; fake_times=[]; fake_prices_txt2=['']; fake_times_txt2=[''];
...
for x in fake_prices_txt2:
    try:
        ...
        aa=[];
cnt=0;
f = open("C:\\Users\Sean\Python Read_Writes Files\\times2.txt","r"); fake_times_txt=f.read();
...
for s in range(10,90,5):
    ...
    start_money=25; current_money=start_money; cnt=0;
        
    #-------------End Test Code-----------------
    
    while current_time<time_min:
        #a=get_price();
        a=fake_prices[cnt];
        ff(float(a));
    ...
    #minute=int(np.floor(10*current_time/cnt)+s)
    minute=47

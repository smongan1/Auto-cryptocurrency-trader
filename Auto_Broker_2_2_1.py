...
def line_test(times,pr_in,s_1,s_2,t_offset):
    m, b, r, p, s=stats.linregress(times,pr_in)
    s=np.std(pr_in)
    ...
    #plt.plot(times[-300:],[(x*m+b)*480  for x in times[-300:]])
...
if True: #Hides test parameters
    fake_prices=[]; fake_times=[]; cnt=0; fake_prices_txt2=['']; fake_times_txt2=['']; 
    f = open("C:\\Users\Sean\Python Read_Writes Files\prices.txt","r"); fake_prices_txt=f.read(); f.close();
    ...
    for x in fake_prices_txt2:
        try:
            ...
            aa=[];
    cnt=0;
    f = open("C:\\Users\Sean\Python Read_Writes Files\\plot_time.txt","r"); fake_times_txt=f.read();f.close()
...
for s in range(0,20):
    if True:  #Hides init parameters
        ...
        test_hour=1;
        owned_crypto=0;
        minute=22+s*5; # 66 seconds is good for set 2 (52 points), 86.5 secs for second set (57 points)
        hour_out_price_h=0;
        ff=prices.append;
        tt=tot.append;
        pp=plot_time.append;
    ...
    while(flag):
        ...
        pp(current_time);
        tt(current_money+owned_crypto*current_price)#total money is stored per unit time
        if np.mod(count,10)==0 or std_all==0:
            all_ave=sum(prices)/len(prices)
            all_std=np.std(prices)
            
        top_price=current_price<(all_ave-0.5*all_std);
        top_price=1;
        ...
        if (limit_price==0 or (current_time-buy_time)>60):
            if (not is_bought):
                ...
                buy_tests=buy_or_tests and enough_money and top_price
        ...
        if count>end_cnt:
            flag=0;
        if False: #np.mod(count,10)==0:
            plt.plot(plot_time[(count-300):count],tot[(count-300):count])
            plt.plot(plot_time[(count-300):count],[x*480 for x in prices[(count-300):count]])
            if limit_price>0:
                plt.plot(plot_time[(count-300):count],[limit_price*480 for x in plot_time[(count-300):count]],plt_v)
            plt.show()
            plt.pause(0.1)

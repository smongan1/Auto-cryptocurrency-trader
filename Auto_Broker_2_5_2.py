# -*- coding: utf-8 -*-
"""
Created on Sun Mar 14 11:05:45 2021
...
p_max=max(max(prices),0.058);
p_min=min(min(prices),0.0566);
...
while(flag):
    ...
    count=count+1
    current_price=prices[-1]
    p_max=max(current_price,p_max);
    p_min=min(current_price,p_min);
    p_ave=(p_min+p_max)/2
    current_time=time.time()-start_time;
    plot_time.append(current_time);
    prices=prices[-(10*minute+10):]
    plot_time=plot_time[-(10*minute+10):]
    ...
    if (limit_price==0 or (current_time-buy_time)>120):
        ...
        if (not is_bought):
            std_d=-std
            bought_price=current_price*2
            thprice, thgprice=p_ave>current_price, p_ave*(1+2*std)>current_price
        else:
            std_d=std*(2.2)
            thprice, thgprice=p_ave<current_price, p_ave*(1+2*std)<current_price
        ...
        tminprice, tgprice=line_test(plot_time[-5*minute:],prices[-5*minute:],std_m,4*std_m,minute)
        tVminprice, tVgprice=line_test(plot_time[-10*minute:],prices[-10*minute:],std_v,4*std_v,minute)
        gpt=tVgprice or tgprice;
        s_test=(tminprice and tVminprice and thprice);
        or_tests=s_test or gpt or (p_ratio>1.003)
